...
// Wait for DOM to be ready before setting up form handler
document.addEventListener('DOMContentLoaded', function() {
    const form = document.querySelector("#contactForm");
    const modalContent = document.querySelector(".modal-content");
...
// Function to setup form handler (for reuse after modal reset)
function setupFormHandler() {
    ...
    if (form && modalContent) {
        form.addEventListener("submit", async (e) => {
            ...
            setTimeout(() => {
                document.getElementById("contactModal").style.display = "none";
                // Reset modal content here if needed
            }, 3000);
        });
    }
}
...
document.addEventListener("DOMContentLoaded", function() {
    const iqrInfoBtn = document.getElementById('iqr-info-btn');
    if (iqrInfoBtn) {
        iqrInfoBtn.addEventListener('click', (e) => {
